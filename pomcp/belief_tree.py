...
from pomcp.helper import rand_choice, round
...
class BeliefNode(Node):
    ...
    def sample_state(self, n_samples = 1):
        if n_samples == 1:
            return rand_choice(self.B)
        elif n_samples > 1:
            if n_samples > len(self.B):
                n_samples = len(self.B)
            samples = random.sample(self.B, n_samples) # without replacement
            #samples = random.choices(self.B, k =n_samples)
